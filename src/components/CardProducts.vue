...
const goDetail = (id) => {
    router.push({ path: 'productos/' + id, params: { id: id } })
}
</script>
<template>
    <v-card class="p-4 m-2" :variant=variants hover>
        <v-img height="400px" :src="imagen" cover></v-img>
        ...
        <v-card-actions>
            <v-btn v-if="!carrito" :disabled=!cantidad @click="onAdd" color="orange-lighten-2"
                text="Agregar a carrito"></v-btn>
        ...
        <v-card-actions>
            <v-btn @click="goDetail(id)" class="text-secondary" text="Ver detalle"></v-btn>
        </v-card-actions>
    </v-card>
</template>